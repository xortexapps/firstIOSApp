 ...
 *  - <OpenLayers.Control>
 */
var mGtiMapModelCacheRead = OpenLayers.Class(OpenLayers.Control, {
     ...
     */
    fetch: function(evt) {
        //this.storage = (this.storage !== null) ? this.storage : window.localStorage;
        if (this.active && this.storage != null &&
            ...
            if (!tile.layer.crossOriginKeyword && OpenLayers.ProxyHost &&
                ...
                url = mGtiMapModelCacheWrite.urlMap[url];
            }
            this.storage.getItem(url, this.storageSuccessCallback, function(error) {}, {event: evt});
        }
    },

    storageSuccessCallback: function(dataURI, arguments) {
        var evt = arguments.event;
        var data = dataURI;
        var tile = evt.tile;
        if (data) {
            tile.url = data;
